...
% albedoFunc = @(X,Y) round(cat(3,interp2(albedoRange,albedoRange,im(:,:,1),X,Y,'cubic'), ...
%     interp2(albedoRange,albedoRange,im(:,:,2),X,Y,'cubic'), ...
%     interp2(albedoRange,albedoRange,im(:,:,3),X,Y,'cubic')));
...
if display_
    ...
    [X,Y] = meshgrid(x,y);
    figure; hold on;
    surf(X,Y,zFunc(X,Y),uint8(albedoBinFunc(X,Y)));
    shading interp
    axis tight
    axis equal
end
